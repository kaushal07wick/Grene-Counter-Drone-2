...
void goal_pose_cb(const geometry_msgs::PoseStamped pose)
{
    ...
    goalPose(2) = pose.pose.position.z;
    ...
    while(!DESTINATION_REACHED || ros::ok()) /** until the goal is reached or the node is killed, keep running the process **/
    {
        ...
        {    
            ...
            if(count == 0 || startOver == 1)
                {
                    x = kAstar.search(startPose, startVel, startAcc, goalPose, goalVel, true, true, 0.0);
                    startOver = 0;
                }                
            else
                {
                    x = kAstar.search(startPose, startVel, startAcc, goalPose, goalVel, false, true, 0.0);
                }
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber pos     = n.subscribe<geometry_msgs::PoseStamped>("/uav0/mavros/local_position/pose",10,local_pose_cb);
    ros::Subscriber goal    = n.subscribe<geometry_msgs::PoseStamped>("/uav1/mavros/local_position/pose",1,goal_pose_cb);
